...
<xsl:output indent="yes" cdata-sectionelements="style"/>
<xsl:template match="/">
<svg>
...
<rect x="{$posicaox}" y="{$posicaoy}" height="{$altura_}" width="{$largura_}" style="fill:blue;stroke:green;stroke-width:5;fillopacity:0.1; stroke-opacity:0.9"/>
</xsl:for-each>
</svg>
</xsl:template>
</xsl:stylesheet>